...
namespace Sinbad.Controllers
{
    ...
    public class EspController : ControllerBase
    {
        ...
        // Function for return all lines of the mac_table
        [HttpGet("mac_table")]
        public async Task<List<MAC>> GetMacTable()
        {
            return await _context.mac_table.ToListAsync();
        }
        ...
        public async Task<IActionResult> Post([FromBody] string data)
        {
            ...
            // We verify that we don't have already this mac address in our table before adding it
            int exist = await _context.mac_table.Where(item => item.mac == macdata.mac).CountAsync();
            if (exist == 0)
            {
                _context.mac_table.Add(macdata);
            }
        ...
        public async Task<IActionResult> Delete(int id = 0, [FromBody] string cleanDataBase = "false")
        {
            ...
            var item_to_remove = await _context.mac_table.FindAsync(id);
            ...
            // Else we remove all datas with mac given by FindAsync in all tables
            else
            {

                await _context.mac_table.Where(item => item.mac == item_to_remove.mac).ExecuteDeleteAsync();
                await _context.temp_sensor_table.Where(item => item.mac == item_to_remove.mac).ExecuteDeleteAsync();
